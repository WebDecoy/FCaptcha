...
PoW challenge/claim state, token replay protection, and Siteverify idempotency;
its detection stores remain local, so it must remain single-instance. Python
does not yet use Redis and must also remain single-instance.
...
| `REDIS_URL` | Share security state across replicas. Go shares all security stores and supports multiple replicas. Node currently shares PoW, token replay, and Siteverify idempotency but must remain single-instance until its detection stores land. Python does not yet use Redis. Configured Redis failures are fail-closed | (unset, process-local state) |
